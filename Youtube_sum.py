...
def generate_summarize(raw_text, callback, language):
    ...
    if language == 'ko':
        system_message = "다음 나올 문서를 'Notion style'로, 적절한 이모지를 사용해서 불렛포인트로 요약해줘. 중요한 내용만 요약하고, 불렛포인트의 이모지는 그 문장의 내용을 반영해야 해. 모든 문장의 끝에 '냥'을 붙여줘. 또한 '~이다냥, ~했다냥'과 같은 자연스러운 문장으로 끝나게 해줘."
        end_text = "\n\n마음에 드냐옹? 💕 언제든 추가로 질문하라냥! 🐾"
